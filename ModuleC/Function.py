...
class Function(QMainWindow):
    ...
    def drawOneLine(self,m :Meta):
        self.ax.plot(m.listx,m.listy,label=m.functionStr)
        self.fig.legend()
        # plt.legend()
        self.canvas.draw()
    ...
    def cheakXNum(self):
        if (self.ui.XEdit.text() == ""):
            ...
            return
        if (int(self.ui.XEdit.text()) < 0):
            self.showMessage("取值范围≥0")
            return
    ...
    def startPlot(self):
        ...
        fig,ax = plt.subplot()
        ax:Axes = ax
        ax.set_xlim(0,float(self.X))
        ax.set_ylim(0,float(self.Y))
        ax.set_xlabel("输入任务数")
        ax.set_ylabel("输出任务总数")
